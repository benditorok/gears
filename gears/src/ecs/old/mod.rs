...
pub struct GearsWorld {
    ...
    component_vecs: Arc<RwLock<Vec<Box<dyn ComponentVec>>>>,
...
impl fmt::Display for GearsWorld {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            ...
            self.component_vecs.read().unwrap().len()
        )
    }
}
...
impl GearsWorld {
    ...
    fn new() -> Self {
        Self {
            ...
            component_vecs: Arc::new(RwLock::new(Vec::new())),
        }
    }

    fn new_entity(&self) -> usize {
        let mut entities_count = self.entities_count.write().unwrap();
        let mut component_vecs = self.component_vecs.write().unwrap();
    ...
    fn add_component_to_entity<ComponentType: 'static>(
        ...
        let mut component_vecs_lock = self.component_vecs.write().unwrap();
        ...
        component_vecs_lock
            .push(Box::new(RefCell::new(new_component_vec)) as Box<dyn ComponentVec>);
    }

    fn borrow_component_vec_mut<ComponentType: 'static>(
        &self,
    ) -> Option<RefMut<Vec<Option<ComponentType>>>> {
        let component_vecs = self.component_vecs.read().unwrap();
        for component_vec in component_vecs.iter() {
            if let Some(component_vec) = component_vec
                .as_any()
